...
SBTNode * maintain(SBTNode * node, int flag) {
    ...
    node->rchild = maintain(node->rchild, 0);
...
// 可在以下自行定义遍历函数
int output(SBTNode *node) {
    node->lchild != NIL &&output(node->lchild);
    printf("%d ", node->data);
    node->rchild != NIL && output(node->rchild);
    return 1;
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        scanf("%d", &a);
        printf("delete %s!\n", delete_tree(binarytree, a) ? "success" : "failed");
    }
    output(binarytree);
    printf("\n");
    clear(binarytree);
